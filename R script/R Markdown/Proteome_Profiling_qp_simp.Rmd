---
title: "Proteome_Profiling_qp_simp"
...
resDf_protein <- read_csv("~/Desktop/Github/proteome-profiling-ecoli/umut/resDf_protein_trT_set00_StcYtcNasAgrNgrMgh_SYAN_baseMgAllMg_baseNaAllNa_ExpAllPhase_noFilter_p1Sf_noNorm.csv")

nameDictionary_RNA_Protein <- read_csv("~/Desktop/Github/proteome-profiling-ecoli/Files/nameDictionary_RNA_Protein.csv")

MyData <- read_csv("~/Desktop/GitHub/proteome-profiling-ecoli/Files/MyData.csv", 
...
Sector %>% left_join(Quantitative_Proteomics, by= 'Gene') %>% 
...
#remove Gene column, change name of gene_name to Gene, and join df to gs

df %>% subset(select = -c(Gene)) -> df
names(df)[names(df) == 'gene_name'] <- 'Gene'
df %>%
  right_join(gs, by = 'Gene') -> a
a %>% 
  subset(select = -c(Sector.y:`1.0397`)) -> a
...
#change the name of columns on data frames to enable joining of tables
names(MyData)[names(MyData) == 'media'] <- 'sample'
